...
int main(void)
{
    ...
    uint8_t results[TEST_NUM][32] = { 0 };
    for (int i = 0; i < TEST_NUM; i++) {
        uint32_t exted[32];
        extend(exted, g_seed[i]);   // extends g_seed[i] to exted
        Words32 extSeed;
        init_seed(extSeed, exted);
        matList_int8 = new BytomMatList;
        matList_int8->init(extSeed);

        iter_mineBytom(g_msg[i], 32, results[i]);
        ...
        if(32==j) {
            printf("test %2d/%2d: PASS\n", i+1, TEST_NUM);
        } else {
            printf("!!!test %2d/%2d: FAIL!!!\n", i+1, TEST_NUM);
        }
    }

    end = clock();
    std::cout << "Avg time : "
              << (double)(end - start) / CLOCKS_PER_SEC / TEST_NUM << "s"
              << std::endl;
